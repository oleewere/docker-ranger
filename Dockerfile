...
RUN cd /root/ && tar zxvf /root/mysql-connector.tar.gz && \
  cp /root/mysql-connector-java-$MYSQL_JAVA_CONNECTOR_VERSION/mysql-connector-java-$MYSQL_JAVA_CONNECTOR_VERSION-bin.jar /usr/share/java/mysql-connector-java.jar
ENV RANGER_SQL_CONNECTOR_JAR /usr/share/java/mysql-connector-java.jar
...
RUN mkdir -p /etc/ranger-admin/conf
ADD conf/log4j.properties $RANGER_ADMIN_PATH/ews/webapp/WEB-INF/log4j.properties
ADD conf/core-site.xml /etc/ranger-admin/conf/core-site.xml
...
ADD conf/ranger-admin-site.xml /etc/ranger-admin/conf/ranger-admin-site.xml
...
RUN chown $RANGER_ADMIN_USER:$RANGER_ADMIN_GROUP /etc/ranger-admin/conf
RUN chown -R $RANGER_ADMIN_USER:$RANGER_ADMIN_GROUP /etc/ranger-admin/conf/*
...
RUN chown $RANGER_ADMIN_USER:$RANGER_ADMIN_GROUP /usr/share/java/mysql-connector-java.jar
RUN chown $RANGER_ADMIN_USER:$RANGER_ADMIN_GROUP /etc/init.d
RUN chown $RANGER_ADMIN_USER:$RANGER_ADMIN_GROUP /etc/rc2.d
RUN chown $RANGER_ADMIN_USER:$RANGER_ADMIN_GROUP /etc/rc3.d
RUN chown $RANGER_ADMIN_USER:$RANGER_ADMIN_GROUP /usr/bin
